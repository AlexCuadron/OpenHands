...
COMMAND="export PYTHONPATH=evaluation/benchmarks/limo:\$PYTHONPATH && poetry run python evaluation/benchmarks/limo/run_infer.py \
  --agent-cls $AGENT \
...
OUTPUT_DIR=$(find evaluation/evaluation_outputs -path "*/LIMO/$AGENT/*" -type d -name "*$EVAL_NOTE*" 2>/dev/null | sort -r | head -n 1)

# If not found, try to find it anywhere under evaluation_outputs
if [ -z "$OUTPUT_DIR" ]; then
  OUTPUT_DIR=$(find . -path "*/evaluation_outputs/*" -path "*/LIMO/$AGENT/*" -type d -name "*$EVAL_NOTE*" 2>/dev/null | sort -r | head -n 1)
fi
...
if [ "$RUN_EVALUATION" = "eval" ]; then
  ...
  if [ -f "$OUTPUT_FILE" ]; then
    ...
    poetry run python evaluation/benchmarks/limo/scripts/analyze_results.py "$OUTPUT_FILE" --output-dir "$OUTPUT_DIR/analysis"
